document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!influencer_id) {
        alert('로그인이 필요합니다. 인플루언서 ID를 찾을 수 없습니다.');
        window.location.href = '/influencer_login.html';
        return;
    }
    ...
    function displayChatRooms(chatRooms) {
        ...
        chatRooms.forEach(room => {
            ...
            roomElement.textContent = `광고주 ID: ${room.ad_id}`;
            roomElement.onclick = () => {
    ...
    function displayMessages(messages) {
        ...
        messages.forEach(msg => {
            ...
            messageElement.classList.add(msg.sender_id === influencer_id ? 'sent' : 'received');
